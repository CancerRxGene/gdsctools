...
class FilterFeatures(object):
    ...
    def filter_by_type(self, type_list=None):
        if type_list == None:
            print(
                "Please enter list of types to keep. Acceptable types include: 'GENE_VARIANT', 'AMPLIFICATION', 'DELETION', 'METHYLATION'."
            )
        else:
    ...
    def filter_by_gene_list(self, gene_list=None):
        if gene_list == None:
            print(
                "Please enter gene list. To use the GDSC1000 core genes, add argument gene_list = 'Core Genes'."
            )
    ...
    def filter_by_tissue_list(self, tissue_list=None):
        if tissue_list == None:
            ...
            if self.dicts in globals():
                print(
                    "Possible dictionary keys include: ", self.dicts.tissue_dict.keys()
                )
    ...
    def filter_by_cosmic_list(self, cosmic_list=None):
        if cosmic_list == None:
            ...
            if self.dicts in globals():
                print(
                    "Possible dictionary keys include: ", self.dicts.cosmic_dict.keys()
                )
        ...
        if cell_line_list == None:
            ...
            if self.dicts in globals():
                print(
                    ...
                    self.dicts.cell_line_dict.keys(),
                )
    ...
    def filter_by_recurrence(self, min_recurrence=3):
        ...
        flatten_cell_lines = self.working_df.groupby(
        ...
        self.working_df = self.working_df[
            self.working_df[feature].isin(self.feature_count[feature])
        ]
    ...
    def make_matrix(self, min_recurrence=3):
        ...
        final_matrix = pd.crosstab(
            self.working_df[feature],
            columns=[
                self.working_df["COSMIC_ID"],
                self.working_df["TISSUE_FACTOR"],
                self.working_df["CELL_LINE"],
                self.working_df["MSI_FACTOR"],
                self.working_df["MEDIA_FACTOR"],
            ],
        )
        final_matrix[final_matrix > 1] = 1
        self.final_matrix = final_matrix.T.reset_index()
        self.final_matrix.to_csv(
            "./data/gdsc_1000_data/features_matrix.csv", index=False
        )
