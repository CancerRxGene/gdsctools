...
class VolcanoANOVA(object):
    ...
    def _get_volcano_sub_data(self, mode, target=None):
        ...
        if mode == self._colname_drugid:
            ...
            raise ValueError(
                "mode parameter must be in [FEATURE, %s, ALL]" % (self._colname_drugid)
            )
        ...
        if self.settings.analysis_type == "PANCAN":
            for sign, qval, pval in zip(signed_effects, qvals, pvals):
                if (
                    sign <= -self.settings.effect_threshold
                    and qval <= FDR_threshold
                    and pval <= self.settings.pvalue_threshold
                ):
                    colors.append("green")
    ...
    def _volcano_plot(self, data, title=""):
        ...
        scatter = ax.scatter(
            X, Y, s=markersize, alpha=0.3, c=colors, linewidth=1, picker=True
        )
        ...
        ax.axhline(
            ...
            color="red",
            alpha=1,
            label="FDR {:.2f} ".format(fdr) + " %",
        )

        for i, this in enumerate(fdrs):
            if (
                this < self.df[self._colname_qvalue].min()
                or this > self.df[self._colname_qvalue].max()
            ):
                continue
        ...
        title_handler = pylab.title(
            "%s" % str(title).replace("_", "  "), fontsize=self.settings.fontsize / 1.2
        )
        labels = []
        ...
        def onpick(event):
            ...
            try:
                title = (
                    str(str(data.iloc[ind]["Drug"]))
                    + " / "
                    + str(data.iloc[ind].Feature)
                )
...
class VolcanoANOVAJS(VolcanoANOVA):
    ...
    def _render_data(self, name="all associations"):
        ...
        text = []
        for x, y, z in zip(
            self.data["Drug"].values,
            self.data["Feature"].values,
            self.data["FDR"].values,
        ):

            text.append(
                "<b>Drug:</b>%s <br><b>Feature:</b>%s <br><b>FDR:</b>%s" % (x, y, z)
            )
        jinja["vars"] = text
        ...
        jinja['FC'] = FC
        """
        self.data.markersize /= self.data.markersize.max() / 3.0

        # First value is Y, second is X, following will be used in the
        try:  # introduced in pandas > 0.16.2
            jinja["data"] = (
                self.data[["signed_effect", "log10pvalue", "markersize"]]
                .round(3)
                .values.tolist()
            )
