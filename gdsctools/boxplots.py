...
class BoxPlots(object):
    ...
    def boxplot_pancan(self, mode, fignum=1, title_prefix=""):
        ...
        if N <= 2:  # msi or 2 tissues
            ...
            fontsize = max(4, int(self.fontsize - (N - 2.0) / (self.fontsize - 4.0)))
        else:
        ...
        self.ax.set_yticklabels(
            [str(len(this)) + " " for this in data], fontsize=fontsize / 1.4
        )
        try:
            pylab.tight_layout()
        except:
        ...
        if self.savefig is True:
            ...
            filename += "ODOF_{}_DRUG_{}____{}".format(mode, self.drug, self.feature)
    ...
    def boxplot_association(self, fignum=1):
        ...
        pylab.title(
            "Individual association\n {0} versus {1}".format(drug_name, self.feature),
            fontsize=self.fontsize,
        )
        pylab.ylabel("{0} logIC50".format(self.drug), fontsize=self.fontsize)
        ...
        if self.savefig is True:
            filename = self.directory + os.sep
            filename += "ODOF_all_DRUG_{}____{}".format(self.drug, self.feature)
            pylab.savefig(filename + ".png", bbox_inches="tight")
    ...
    def _get_boxplot_data(self, mode="tissue"):
        ...
        groups = df.query(mode + " in @categories", engine="python").groupby(
            [mode, "feature"]
        )
        ...
        if len(means):  # need 2 values
            ...
            for category in delta.loc["ic50"].index:
                ...
                neg = df.query(prefix_query + " and feature==0", engine="python")[
                    "ic50"
                ]
                pos = df.query(prefix_query + " and feature==1", engine="python")[
                    "ic50"
                ]
...
class BoxPlotsJS(BoxPlots):
    def __init__(self, odof, fontsize=20, savefig=False, directory="."):
        super(BoxPlotsJS, self).__init__(
            odof, fontsize=fontsize, savefig=savefig, directory=directory
        )
