...
class RegressionReport(object):
    ...
    def __init__(
        ...
        self.drugids = [
            this.rstrip(".png").lstrip(self.prefix_images).lstrip("boxplot_")
    ...
    def create_html_main(self, onweb=False):
        ...
        if self.verbose:
            logger.info("Creating main HTML page in directory %s" % (self.directory))
...
class HTMLOneDrug(ReportMain):
    ...
    def _create_report(self, onweb=True):
        ...
        text = {}
        text["boxplot"] = (
            "This boxplot shows the %s most important features "
            "(based on the weights of the regression)."
        )
        text["boxplot"] %= self.caller.config["boxplot_n"]

        text["importance"] = (
            "Feature with non-null weights. If empty, it"
        ...
        for this in ["boxplot", "randomness", "importance", "weights"]:
            ...
            filename = (
                self.caller.prefix_images + "%(name)s_%(drugid)s.png" % self.params
            )
...
class HTMLPageMain(ReportMain):
    ...
    def _create_report(self, onweb=True):
        ...
        table.df["drugid"] = [
            '<a href="drug_%s.html">%s</a>' % (x, x) for x in table.df["drugid"]
        ]
