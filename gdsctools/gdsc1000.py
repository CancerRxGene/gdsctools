...
class GDSC1000( object ):
    ...
    def _combine_genomic_annotation( self, save_csv ):
        ...
        self.genomic_df = pd.concat( [self.variant_df, self.cna_df, self.methylation_df] , axis = 0, join = 'inner' )
        self.genomic_df = self.genomic_df.merge( self.cell_line_dict, how = 'left', on = ['CELL_LINE', 'TISSUE_FACTOR'])
