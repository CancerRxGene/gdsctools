...
class GDSC1000(object):
    ...
    def __init__(self, annotate=False, data_folder_name="./data/gdsc_1000_data/"):
        ...
        self._url_base = (
            "http://www.cancerrxgene.org/gdsc1000/GDSC1000_WebResources/Data/suppData/"
        )
    ...
    def _combine_genomic_annotation(self):
        ...
        self.genomic_df = pd.concat(
            [self.variant_df, self.cna_df, self.methylation_df], axis=0, join="inner"
        )
        self.genomic_df = self.genomic_df.merge(
            self.cell_line_df, how="left", on=["CELL_LINE", "TISSUE_FACTOR"]
        )
    ...
    def _string_split(self, to_be_split, col_name, separator):
        ...
        s.index = s.index.droplevel(
            -1
        )  # Flattens levels to retrieve indices from original frame
        # Defining name
    ...
    def _read_annotation(self):
        ...
        self.annotation_df = pd.concat(
            [methylation_annotation, cna_annotation, variant_annotation],
            axis=0,
            join="inner",
        )
    ...
    def _check_filter(self, values, valid_values):
        if values is None:
            raise TypeError(
                "Please enter list of types to keep. Valid types: "
                + ", ".join(valid_values)
            )
        for this in values:
            if this not in valid_values:
                raise ValueError(
                    "%s not valid. Use one of " % this + " ".join(valid_values)
                )
    ...
    def filter_by_gene(self, gene_list=None):
        if gene_list is None:
            print(
                "Please enter a valid gene list or set to 'Core Genes' "
    ...
    def filter_by_tissue_type(self, tissue_list=None):
        if tissue_list == None:
            print(
                "Please enter list of tissues. To use all tissues, add argument tissue_list = 'Complete'"
            )
            print(
                "Possible dictionary keys include: ", self.defaults.tissue_dict.keys()
            )
            return
    ...
    def filter_by_cosmic_id(self, cosmic_list=None):
        if cosmic_list == None:
            print(
                "Please enter list of COSMIC IDs. To use an example set of COSMIC IDs, add argument cosmic_list = 'Dummy'"
            )
            print(
                "Possible dictionary keys include: ", self.defaults.cosmic_dict.keys()
            )
            return
    ...
    def filter_by_cell_line(self, cell_line_list=None):
        if cell_line_list == None:
            print(
                "Please enter list of cell line names. To use an example set of names, add argument cell_line_list = 'Dummy'"
            )
            print(
                "Possible dictionary keys include: ",
                self.defaults.cell_line_dict.keys(),
            )
            return
    ...
    def filter_by_recurrence(self, min_recurrence=None):
        ...
        flatten_cell_lines = self.genomic_df.groupby(
            [feature, "COSMIC_ID", "TISSUE_FACTOR"], as_index=False
        )[["ALTERATION_TYPE"]].count()
        feature_count = flatten_cell_lines.groupby([feature], as_index=False)[
            ["COSMIC_ID"]
        ].count()
        self.feature_count = feature_count[feature_count["COSMIC_ID"] >= min_recurrence]

        self.genomic_df = self.genomic_df[
            self.genomic_df[feature].isin(self.feature_count[feature])
        ]
    ...
    def make_matrix(self, min_recurrence=None):
        ...
        genomic_matrix = pd.crosstab(
            self.genomic_df[feature],
            columns=[
                self.genomic_df["COSMIC_ID"],
                self.genomic_df["TISSUE_FACTOR"],
                self.genomic_df["CELL_LINE"],
                self.genomic_df["MSI_FACTOR"],
                self.genomic_df["MEDIA_FACTOR"],
            ],
        )
        genomic_matrix[genomic_matrix > 1] = 1
        self.genomic_matrix = genomic_matrix.T.reset_index()
        self.genomic_matrix.MSI_FACTOR = self.genomic_matrix.MSI_FACTOR.map(
            msi_dictionary
        )

        self._to_csv(
            self.genomic_matrix,
            "genomic_features_make_matrix.csv",
            quoting=csv.QUOTE_NONNUMERIC,
        )

        ic50_filtered_matrix = self.ic50_df.query(
            "COSMIC_ID in @self.genomic_matrix.COSMIC_ID.tolist()"
        )
