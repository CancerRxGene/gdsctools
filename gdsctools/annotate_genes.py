...
class MergeGDSCData(object):
    ...
    def merge_all(self):
        ...
        self.complete_df = pd.concat(
            [self.variant_df, self.cna_df, self.methyl_df], axis=0, join="inner"
        )
    ...
    def _download_annotation(self):
        ...
        urlrequest.urlretrieve(
            methylation_annotation_url,
            self.data_folder_name + "methylation_annotation.xlsx",
        )
        urlrequest.urlretrieve(
            cna_annotation_url, self.data_folder_name + "cna_annotation.xlsx"
        )
    ...
    def _annotate_methylation(self):
        ...
        methyl_annotation = methyl_annotation[
            ["Cancer Types", "Genomic Coordinates", "GN"]
        ]
        methyl_annotation.index = range(len(methyl_annotation.index))
        methyl_annotation.columns = ["TISSUE_FACTOR", "IDENTIFIER", "GENE"]
        methyl_df = methyl_df.merge(
            methyl_annotation, how="left", on=["IDENTIFIER", "TISSUE_FACTOR"]
        )
    ...
    def _annotate_cna(self):
        ...
        cna_df = cna_df.merge(
            cna_annotation, how="left", on=["IDENTIFIER", "TISSUE_FACTOR"]
        )
